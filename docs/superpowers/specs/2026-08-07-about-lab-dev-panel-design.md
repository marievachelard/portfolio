...
The values have to reach call sites in both `SpecSheetColumns.tsx` and `SpecSheetGrid.tsx`, so `SpecSheetColumns.tsx` owns the `DevPanelValues` state itself (`useState`, seeded and persisted as described above) and passes `{ values, onChange }` down into `<SpecSheetDevPanel />` as props. `SpecSheetDevPanel` is a controlled panel — sliders plus the localStorage persistence effect — not the owner of the data. `SpecSheetColumns.tsx` then uses `values` directly wherever the corresponding static class currently sits, and passes the subset `SpecSheetGrid` needs (`marginTop/Bottom/Left/Right`, `imageFlankGap`) down to it as additional props alongside the existing `closing`/`restAt`.

Every knob currently expressed as a Tailwind class becomes an inline `style` pixel value at its call site — Tailwind's build-time class scanner can't see a class name assembled from a runtime number, so `left-20` becomes `style={{ left: values.marginLeft }}` (React appends `px` to bare numbers automatically) for exactly the ten sites these knobs control, and only those ten; everything else on the page keeps its existing static classes untouched.
